    ...
    <section class="content container-fluid">
        ...
        <div class="container-fluid">
            <div class="chart-wrapper">
              ...
              // Requête pour récupérer le nombre total des Enseignants
              $totalQuery = "
                  SELECT COUNT(*) AS totalEnseignant
                  FROM votant 
                  WHERE fonction = 'Enseignant'
              ";

              $totalResult = $conn->query($totalQuery);
              $totalEnseignant = 0; // Valeur par défaut si aucun résultat
              if ($totalResult->num_rows > 0) {
                  $totalRow = $totalResult->fetch_assoc();
                  $totalEnseignant = $totalRow['totalEnseignant']; // Total dynamique des Enseignants
              }

              // Requête pour récupérer les statistiques des votes pour les Enseignants
              $query = "
                  SELECT 
              ...
              if ($result->num_rows > 0) {
                  ...
                  // Calcul des pourcentages selon la règle de 90%
                  $basePourcentage = 90;

                  $pourcentageFavorable = ($favorable / $totalEnseignant) * $basePourcentage;
                  $pourcentageIndecis = ($indecis / $totalEnseignant) * $basePourcentage;
                  $pourcentageOpposant = ($opposant / $totalEnseignant) * $basePourcentage;
                  $pourcentageNonTraite = ($nonTraite / $totalEnseignant) * $basePourcentage;
